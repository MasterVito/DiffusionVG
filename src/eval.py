...
from src.utils import compute_average_precision_detection, compute_temporal_iou_batch_cross, compute_temporal_iou_batch_paired, load_jsonl, get_ap
...
def compute_mr_ap(submission, ground_truth, iou_thds=np.linspace(0.3, 0.95, 14),
...
def compute_recall_at_k(results, ground_truth, iou_thds=np.linspace(0.1, 0.9, 9), k=1):
    ...
    for gt_query in gt_query2span:
        if len(pred_query2span[gt_query])>1:
            pdb.set_trace()
    ...
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    for thd in iou_thds:
        iou_thd2recall_at_k[str(thd)] = float(f"{np.mean(max_ious >= thd) * 100:.2f}")
...
def compute_mr_r5(submission, ground_truth, iou_thds=np.linspace(0.3, 0.95, 14)):
    ...
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    pred_qid2window = {d["qid"]: [k[:2] for k in d["pred_relevant_windows"][:5]] for d in submission}  # :2 rm scores
    ...
    for d in ground_truth:
        ...
        if len(cur_gt_windows) > 0:  # select the GT window that has the highest IoU
            cur_ious = compute_temporal_iou_batch_cross(np.squeeze(np.array([pred_qid2window[cur_qid]])), np.array(d["relevant_windows"]))[0]
            cur_max = cur_ious.max()
            max_ious.append(cur_max)

    max_ious = np.asarray(max_ious)
    iou_thd2recall_at_5 = {}
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    for thd in iou_thds:
        iou_thd2recall_at_5[str(thd)] = float(f"{np.mean(max_ious >= thd) * 100:.2f}")
    miou = np.mean(max_ious)
    return iou_thd2recall_at_5
...
def eval_moment_retrieval(submission, ground_truth, verbose=True):
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        if len(_submission) == 0:
            continue
        print(f"{name}: {l_range}, {len(_ground_truth)}/{len(ground_truth)}="f"{100*len(_ground_truth)/len(ground_truth):.2f} examples.")
        ...
        if len(_submission[0]['pred_relevant_windows'])>=5:
            iou_thd2recall_at_five = compute_mr_r5(_submission, _ground_truth)
        else:
            iou_thd2recall_at_five = {'0.5': 0.0, '0.55': 0.0, '0.6': 0.0, '0.65': 0.0, '0.7': 0.0, '0.75': 0.0, '0.8': 0.0, '0.85': 0.0, '0.9': 0.0, '0.95': 0.0}
        ret_metrics[name] = {"MR-mAP": iou_thd2average_precision, "MR-R1": iou_thd2recall_at_one, "MR-R5": iou_thd2recall_at_five}
        if verbose:
