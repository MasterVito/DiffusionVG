...
def save_json_full(data, epoch_i, filename, save_pretty=False, sort_keys=False):
    data = data["full"]
    for k in data.keys(): data[k] = {"0.3": data[k]["0.3"], "0.5": data[k]["0.5"], "0.7": data[k]["0.7"]}
    with open(filename, "a") as f:
        f.write(json.dumps({f"Epoch-{epoch_i+1:03d}":data}, sort_keys=sort_keys) + "\n")
